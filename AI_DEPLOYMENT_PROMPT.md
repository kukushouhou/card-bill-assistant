...
5. 不执行 `docker compose down -v`、不删除数据卷、不重置数据库。已有部署升级时必须保留原 `.env` 和 `ENCRYPTION_KEY`。备份询问由目标版本是否包含数据库变更决定，不由内置或外置 MySQL 决定：全新安装不询问备份；升级但不涉及数据库变更时也不询问；只有升级且将执行数据库结构或数据迁移时，才询问用户是否先备份，并推荐备份。
...
4. **镜像来源**：
   - 推荐默认：直接拉取 `ghcr.io/kukushouhou/card-bill-assistant:0.3.1`，部署快且构建结果与发布版本一致。
...
| 镜像来源 | GHCR `0.3.1` | 固定版本便于回滚；不以 `latest` 作为生产默认值 |
| `APP_VERSION` | `0.3.1` | Compose 拉取的 GHCR 镜像标签 |
...
6. 仅在已有部署升级时，先根据当前版本、目标版本发布说明和迁移文件判断本次升级是否会执行数据库结构或数据迁移：
   - 没有数据库变更：不要询问数据库备份，直接继续升级准备。
   - 存在数据库变更：询问用户是否在升级前备份，并将“先备份”作为推荐选项；内置和外置 MySQL 使用同一判断规则。
   - 用户不备份：说明迁移失败或回滚时可能无法恢复升级前数据，记录用户选择后继续，不再追问备份位置。
   - 用户确认备份：此时才检查数据库连通性、导出工具、账号读取/导出权限和可用磁盘空间，并询问备份保存目录与文件名。推荐使用带日期时间的新文件，避免覆盖已有备份。只有选定目标已经存在时，才询问改用新文件名还是覆盖；未经用户明确选择不得覆盖。
   - 执行备份后检查命令退出状态和备份文件非空，再继续会触发数据库迁移的启动操作。连接账号密码可用于内置或外置 MySQL 备份，但必须实际具备目标数据库的导出权限。
7. 先运行 `pull`，再启动官方镜像：
...
- 已有部署升级时先保留原 `.env`，并在启动目标版本前判断是否存在待执行的数据库变更。只有存在数据库变更时才询问是否备份；用户确认后再收集备份保存信息，并只在选定目标已存在时询问是否覆盖。这一流程同时适用于内置和外置 MySQL。明确修改 `APP_VERSION` 后执行 `pull` 与 `up -d`；源码构建部署才重新执行 `--build`。容器启动时会自动执行 `prisma migrate deploy`，因此条件性备份必须在启动目标版本前完成。
