...
4. **镜像来源**：
   - 推荐默认：直接拉取 `ghcr.io/kukushouhou/card-bill-assistant:0.2.3`，部署快且构建结果与发布版本一致。
...
| 镜像来源 | GHCR `0.2.3` | 固定版本便于回滚；不以 `latest` 作为生产默认值 |
| `APP_VERSION` | `0.2.3` | Compose 拉取的 GHCR 镜像标签 |
