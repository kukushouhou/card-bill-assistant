...
export default function Bills() {
  ...
  const searchField = <Input.Search aria-label="搜索账单和提醒" placeholder="搜索银行、卡尾或提醒" allowClear value={search} onChange={event => setSearch(event.target.value)} onSearch={q => update({ q: q.trim() })} />;
  const fields = <div className="agenda-filters">
    <Select variant={isMobile ? 'outlined' : 'borderless'} aria-label="银行" placeholder="全部银行" allowClear value={params.get('bank') ?? undefined}
    ...
    <Select variant={isMobile ? 'outlined' : 'borderless'} aria-label="卡片" placeholder="全部卡片" allowClear showSearch optionFilterProp="label" value={params.get('cardId') ? Number(params.get('cardId')) : undefined}
    ...
    <Select variant={isMobile ? 'outlined' : 'borderless'} aria-label="记录类型" placeholder="全部类型" allowClear value={params.get('kind') ?? undefined} onChange={kind => update({ kind })} options={[
    ...
    {(isMobile || ['bank', 'cardId', 'cardIds', 'kind', 'q'].some(key => params.has(key))) && <Button type={isMobile ? 'default' : 'text'} onClick={() => { setSearch(''); setParams({ view }, { replace: true }); }}>清除筛选</Button>}
  </div>;
  ...
  const controls = <>
    <div className="agenda-toolbar agenda-navigation">
      ...
      {isMobile && settingsActions}
      {!isMobile && <div className="agenda-table-search">{searchField}</div>}
    </div>
    {!isMobile && fields}
  ...
  return <Page title="账单中心" className="bill-center" extra={settingsActions}>
    {isMobile && controls}
    <AgendaPage query={query.toString()} revision={revision} controls={isMobile ? undefined : controls} />
