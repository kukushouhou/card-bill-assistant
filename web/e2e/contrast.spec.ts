  ...
  test('主要文字对比度 ' + skin + ' ' + mode, async ({ page, request }) => {
    ...
    for (const width of [1440, 390]) for (const route of ['/', '/bills', '/settings', '/parsers']) {
      ...
      const result = await page.evaluate(() => {
        ...
        const elements = document.querySelectorAll<HTMLElement>('h1,h2,h3,.ant-tag,.ant-typography-warning,.ant-typography-danger,.ant-statistic-content-value,.ant-card-head-title,.agenda-identity strong,.agenda-identity>span,.agenda-amount,.agenda-amount-state,.agenda-dates,.agenda-summary-title,.agenda-summary-value,.agenda-summary-metrics li,.agenda-summary-currency-name,.agenda-summary-currency dt,.agenda-summary-currency-code,.agenda-summary-notice-heading,.agenda-summary-notes li,.ant-form-item-label>label,.ant-typography-secondary,.ant-btn>span:not(.anticon),.ant-menu-title-content,.ant-segmented-item-label,.mobile-todo-card-due strong,.mobile-dashboard-key-stat strong,.ant-radio-button-wrapper-checked>span:last-child,.ant-switch-inner-checked');
