...
- 当前稳定版本：`v0.5.0`（[查看本版本完整发布说明](./docs/releases/v0.5.0.md)）
- 官方容器镜像：`ghcr.io/kukushouhou/card-bill-assistant:0.5.0`
...
- 电脑端点击信用卡记录的前两列任意位置，手机端点击账单主体，或使用「明细」按钮即可进入。没有本期账单时直接按该卡筛选历史使用记录，不会禁用入口。
- 账单中心、卡片详情和首页年费入口共用明细页；即使该账单暂无明细，也可以进入并继续查看历史。
- 「本账单」顶部显示应还总额、还款状态、还款日和最低还款额，部分还款同时显示已还与剩余待还；金额来自账单记录，不是当前页交易小计。
...
- 安装向导和系统设置均支持添加多份同类型配置（例如两台手机各配置一个 Bark），每份配置可独立命名、编辑、启停、删除和测试。所有启用渠道会收到当天提醒。
- Bark 的高级设置默认折叠，支持推送分组、可搜索的内置铃声、通知级别和图标；已在手机 Bark 中导入的铃声可通过次级入口填写名称。设置页可直接测试尚未保存的配置，测试不会保存或启用渠道。
- 自定义 HTTP 推送默认只需填写请求方法、URL 和基础参数；展开高级设置后可配置查询参数、请求头、JSON/表单/纯文本正文及模板。模板支持 `{{title}}`、`{{body}}`、`{{group}}`、`{{count}}`、`{{appName}}` 占位符，不执行用户脚本。
- 通知渠道配置加密保存；编辑退出时保护未保存草稿，桌面弹窗与手机全屏流程共用同一份草稿。
...
| 变量 | 必填 | 默认值 | 用途 |
| --- | :---: | --- | --- |
| `APP_VERSION` | 否 | `0.5.0` | GHCR 镜像版本；生产环境建议固定版本 |
