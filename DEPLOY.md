...
当前发布版本为 `v0.5.0`，官方多架构镜像为 `ghcr.io/kukushouhou/card-bill-assistant:0.5.0`，支持 `linux/amd64` 与 `linux/arm64`。生产环境建议固定 `APP_VERSION`，不要长期跟随 `latest`。
...
```bash
git clone --branch v0.5.0 --depth 1 https://github.com/kukushouhou/card-bill-assistant.git
